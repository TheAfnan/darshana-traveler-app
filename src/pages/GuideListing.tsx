    ...
    <div className={`min-h-screen transition-colors duration-300 font-sans pb-20 ${
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-3 max-w-3xl">
              <div className={`inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-xs font-bold ${
                ...
                <ShieldCheck size={13} className={isDarkMode ? 'text-amber-400' : 'text-amber-700'} />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        ...
        {filteredGuides.length === 0 ? (
          <div className={`py-16 text-center rounded-3xl border p-8 space-y-4 shadow-sm ${
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {featuredGuide && (
              <div
                ...
                className="md:col-span-2 rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 relative h-[440px] group cursor-pointer border border-stone-200 dark:border-slate-800"
              >
                <img
            ...
            {bentoRemainingGuides.map((guide) => (
              <div
                key={guide._id}
                onClick={() => setSelectedGuide(guide)}
                className="rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 relative h-[420px] group cursor-pointer border border-stone-200 dark:border-slate-800 flex flex-col justify-end"
              >
                <img
                  src={guide.profileImage}
                  alt={guide.name}
        ...
        {selectedGuide && (
          ...
          >
            <motion.div
              ...
              className={`rounded-3xl max-w-xl w-full overflow-hidden shadow-2xl border my-8 ${
                isDarkMode 
              ...
              <div className="p-6 space-y-5 max-h-[65vh] overflow-y-auto">
                ...
                {bookingSuccess ? (
                  <div className="p-6 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-2xl text-center space-y-2">
                    ...
                    <h4 className={`font-bold text-base ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Request Sent!</h4>
                    <p className="text-xs text-slate-500">
                      {selectedGuide.name} will contact you shortly to confirm your booking.
                    </p>
                  </div>
                ) : (
                  <form onSubmit={handleBookingSubmit} className="pt-3 border-t border-stone-200 dark:border-slate-800 space-y-4">
                    <h4 className={`text-xs font-bold uppercase ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
